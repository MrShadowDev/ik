...
def sendMessage():
    ...
    stop_msg = Fore.RED + ("Press 'space' to stop advertising!" if os.name == 'nt' else "Enter 'space' to stop advertising!")
    print(stop_msg)
    while True:
        if (os.name == 'nt' and 'msvcrt' in sys.modules and msvcrt.kbhit() and msvcrt.getch() == b' ') or (os.name != 'nt' and select.select([sys.stdin,],[],[],0.0)[0] and sys.stdin.readline().strip() == ' '):
            main()
            break
        ...
        for channel in channels:
            ...
            x+=1;subprocess.call('title="Discord AdBot | Messages Sent: {}"'.format(x), shell=True)
...
x=0;subprocess.call('title="Discord AdBot | Messages Sent: {}"'.format(x), shell=True)
main()